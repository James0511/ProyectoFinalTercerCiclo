...
public class Intelligence {
    ...
    public String recognize(CarSnapshot carSnapshot) throws Exception {
        ...
        if (enableReportGeneration) {
            ControlCamara.rg.insertText("<h1>Automatic Number Plate Recognition Report</h1>");
            ControlCamara.rg.insertText("<span>Image width: "+carSnapshot.getWidth()+" px</span>");
            ControlCamara.rg.insertText("<span>Image height: "+carSnapshot.getHeight()+" px</span>");
            
            ControlCamara.rg.insertText("<h2>Vertical and Horizontal plate projection</h2>");
            
            ControlCamara.rg.insertImage(carSnapshot.renderGraph(), "snapshotgraph",0,0);
            ControlCamara.rg.insertImage(carSnapshot.getBiWithAxes(), "snapshot",0,0);
        }
        
        for (Band b : carSnapshot.getBands()) { //doporucene 3

            if (enableReportGeneration) {
                ControlCamara.rg.insertText("<div class='bandtxt'><h4>Band<br></h4>");
                ControlCamara.rg.insertImage(b.getBi(),"bandsmall", 250,30);
                ControlCamara.rg.insertText("<span>Band width : "+b.getWidth()+" px</span>");
                ControlCamara.rg.insertText("<span>Band height : "+b.getHeight()+" px</span>");
                ControlCamara.rg.insertText("</div>");
            }
            
            for (Plate plate : b.getPlates()) {//doporucene 3

                if (enableReportGeneration) {
                    ControlCamara.rg.insertText("<div class='platetxt'><h4>Plate<br></h4>");
                    ControlCamara.rg.insertImage(plate.getBi(),"platesmall", 120, 30);
                    ControlCamara.rg.insertText("<span>Plate width : "+plate.getWidth()+" px</span>");
                    ControlCamara.rg.insertText("<span>Plate height : "+plate.getHeight()+" px</span>");
                    ControlCamara.rg.insertText("</div>");
                }                
                ...
                if (enableReportGeneration) {
                    ControlCamara.rg.insertText("<h2>Detected band</h2>");
                    ControlCamara.rg.insertImage(b.getBiWithAxes(),"band",0,0);
                    ControlCamara.rg.insertImage(b.renderGraph(),"bandgraph",0,0);
                    ControlCamara.rg.insertText("<h2>Detected plate</h2>");
                    Plate plateCopy = plate.clone();
                    plateCopy.linearResize(450, 90);
                    ControlCamara.rg.insertImage(plateCopy.getBiWithAxes(), "plate",0,0);
                    ControlCamara.rg.insertImage(plateCopy.renderGraph(), "plategraph",0,0);
                }
                
                // SKEW-RELATED
                if (enableReportGeneration) {
                    ControlCamara.rg.insertText("<h2>Skew detection</h2>");
                    //Main.rg.insertImage(notNormalizedCopy.getBi());                    
                    ControlCamara.rg.insertImage(notNormalizedCopy.getBi(), "skewimage",0,0);
                    ControlCamara.rg.insertImage(renderedHoughTransform, "skewtransform",0,0);
                    ControlCamara.rg.insertText("Detected skew angle : <b>"+hough.angle+"</b>" );
                }
                ...
                if (enableReportGeneration) {
                    ControlCamara.rg.insertText("<h2>Character segmentation</h2>");
                    ControlCamara.rg.insertText("<div class='charsegment'>");
                    for (Char chr : chars) {
                        ControlCamara.rg.insertImage(Photo.linearResizeBi(chr.getBi(),70,100), "",0,0);
                    }
                    ControlCamara.rg.insertText("</div>");
                }
                ...
                for (Char chr : chars) {
                    ...
                    if (enableReportGeneration) {
                        ControlCamara.rg.insertText("<div class='heuristictable'>");
                        ControlCamara.rg.insertImage(Photo.linearResizeBi(chr.getBi(),chr.getWidth()*2, chr.getHeight()*2), "skeleton",0,0);
                        ControlCamara.rg.insertText("<span class='name'>WHR</span><span class='value'>"+widthHeightRatio+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>HEI</span><span class='value'>"+heightCost+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>NEU</span><span class='value'>"+similarityCost+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>CON</span><span class='value'>"+contrastCost+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>BRI</span><span class='value'>"+brightnessCost+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>HUE</span><span class='value'>"+hueCost+"</span>");
                        ControlCamara.rg.insertText("<span class='name'>SAT</span><span class='value'>"+saturationCost+"</span>");
                        ControlCamara.rg.insertText("</table>");
                        if (errorFlags.length()!=0) ControlCamara.rg.insertText("<span class='errflags'>"+errorFlags+"</span>");
                        ControlCamara.rg.insertText("</div>");
                    }
                } // end for each char
                ...
                if (enableReportGeneration) {
                    ControlCamara.rg.insertText("<span class='recognized'>");
                    ControlCamara.rg.insertText("Recognized plate : "+parsedOutput);
                    ControlCamara.rg.insertText("</span>");
                }
